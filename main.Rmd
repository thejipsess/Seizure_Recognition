...
z_train_y <- z_train$y
z_test_y <- z_test$y
z_train <- subset(z_train, select = -y)
z_test <- subset(z_test, select = -y)
...
test_results <- 
  z_test_y %>%
  bind_cols(
    predict(svm_fit, new_data = z_test)) %>%
...
# Setup bootstrapping resample data z_rs
z_rs <- bootstraps(z_train, times = 100)
