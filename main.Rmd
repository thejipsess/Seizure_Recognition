...
split_data <- split_test_train(x, y)
...
max_test_calss <- max(table(split_data$test$y))
...
x_bin_test <- ovun.sample(y ~ ., split_data$test, method = 'over', N = max_test_calss * length(classes), )$data
...
# Perform gridsearch
gridsearch_svm(x_bin_train, SVM_recipe)
...
correctly_classified_test <- test_results$'Predicted class' == test_results$'True class'
...
SVM_recipe <- prep(SVM_recipe, x_bin_train)
train_norm <- bake(SVM_recipe, x_bin_rain)

# Create svm model
svm <- svm(y ~ ., train_norm, scale = FALSE, type = "nu-classification", kernel = 'linear',  cost = 1)

pred <- predict(svm, train_norm)
